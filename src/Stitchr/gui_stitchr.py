...
def switch_receptors(text, current_receptor):
    ...
    for change in changes:
        if current_receptor == "TRA/TRB":
            ...
            raise ValueError(
                "Unknown receptor configuration detected: " + current_receptor
            )
...
def change_receptors(receptor_str):
    ...
    window["link_order_choice"].update(
        values=link_orders[new_receptor], value=link_orders[new_receptor][1]
    )
                ...
                if line_count == 0:
                    if bits != th.in_headers[receptor_type]:
                        ...
                        if (
                            bits
                            == th.in_headers[
                                switch_receptors(receptor_type, receptor_type)
                            ]
                        ):
                ...
                # Then use the data line to replace any matching entries on the webform
                    ...
                    for x in range(len(th.in_headers[receptor_type])):
                        ...
                        # Then only update those fields that are shared
                            ...
                            window[
                                th.locus_to_trx(th.in_headers[receptor_type][x])
                            ].update(bits[x])
                        ...
                        # Update link order...
                            ...
                            if bits[x]:
                                if bits[x] == "AB":
                                    ...
                                    raise warnings.warn(
                                        "Invalid link order: " + bits[x]
                                    )

                                window["link_order_choice"].update(
                                    values=link_orders[receptor_type],
                        ...
                        # ... and linker sequence
                            ...
                            if bits[x]:
                                if bits[x] in linkers.keys():
                                    ...
                                    window["custom_linker"].update(
                                        bits[x], visible=True
                                    )

                elif line_count > 1:
                    warnings.warn(
                        "More than one data line detected in input TCR file. Ignoring lines after first."
                    )
                    break
...
def tidy_values(reference_chain, set_values):
    ...
    for field in ["V", "J", "_CDR3", "_leader", "C"]:
        if set_values[reference_chain + field]:
            set_values[reference_chain + field] = set_values[
                reference_chain + field
            ].upper()
...
def main():
    ...
    global link_orders
    link_orders = {"TRA/TRB": ["AB", "BA"], "TRG/TRD": ["GD", "DG"]}
    ...
    col1 = [
        [
            sg.Button("Example data", size=quart_sz),
            sg.Button("Reset form", size=quart_sz),
        ],
        [
            sg.FileBrowse(
                key="uploaded_tcr", size=quart_sz, button_text="Find TCR input file"
            ),
        ...
        [
            sg.Button(
                ...
                size=half_sz,
                enable_events=True,
                button_text="Change to TRG/TRD",
            )
        ],
        [sg.Text("Additional genes", size=half_sz, font=(fnt, 12))],
        [
            sg.MLine(
                default_text=extra_gene_text,
                size=(int(box_width / 2 - 1.3), 3),
                key="additional_genes",
            )
        ],
        [sg.Input(key="find_preferred_alleles", enable_events=True, visible=False)],
        [
            sg.FileBrowse(
                target="find_preferred_alleles",
                size=half_sz,
                key="preferred_allele_button",
                button_text=preferred_button_default,
            )
        ],
        [
            sg.Checkbox(
        ...
        [
            ...
            sg.Combo(
                link_orders[receptor],
                key="link_order_choice",
                default_value=link_orders[receptor][1],
                size=(8, 1),
                enable_events=True,
            ),
        ],
    ...
    col2 = [
        ...
        [sg.Text("TRAV", key="TR1_V_text")],
        [sg.InputText("", key="TR1V", size=sz)],
        [sg.Text("TRAJ", key="TR1_J_text")],
        [sg.InputText("", key="TR1J", size=sz)],
        [sg.Text("TRA CDR3 junction", key="TR1_CDR3_text")],
        [sg.InputText("", key="TR1_CDR3", size=sz)],
        [sg.Text("TRA name", key="TR1_name_text")],
        [sg.InputText("", key="TR1_name", size=sz)],
        [
            sg.Text("TRA leader", size=half_sz, key="TR1_l_title_text"),
            sg.Text("TRAC", key="TR1_c_title_text"),
        ],
        [
    ...
    col3 = [
        ...
        [sg.Text("TRBV", key="TR2_V_text")],
        [sg.InputText("", key="TR2V", size=sz)],
        [sg.Text("TRBJ", key="TR2_J_text")],
        [sg.InputText("", key="TR2J", size=sz)],
        [sg.Text("TRB CDR3 junction", key="TR2_CDR3_text")],
        [sg.InputText("", key="TR2_CDR3", size=sz)],
        [sg.Text("TRB name", key="TR2_name_text")],
        [sg.InputText("", key="TR2_name", size=sz)],
        [
            sg.Text("TRB leader", size=half_sz, key="TR2_l_title_text"),
            sg.Text("TRBC", key="TR2_c_title_text"),
        ],
        [
    ...
    while True:
        ...
        if event == "Example data":
            example_files = [
                x
                for x in os.listdir(examples_path)
                if x.endswith("tsv") and x[0] not in [".", "~", "_"]
            ]
            example_matches = [
                x
                for x in example_files
                if species in x.upper() and receptor.replace("/", "-") in x
            ]
            ...
            if len(example_matches) == 0:
                sg.Popup(
                    "No example "
                    + receptor
                    + " TCR example files available for species "
                    + species
                    + "."
                )
                ...
                receptor, species = upload_tcr_details(
                    os.path.join(examples_path, example_matches[0]), receptor, species
                )
            ...
            window["link_order_choice"].update(
                values=link_orders[receptor], value=link_orders[receptor][1]
            )
            ...
            receptor, species = upload_tcr_details(
                values["uploaded_tcr"], receptor, species
            )
            ...
            if values["additional_genes"] != extra_gene_text + "\n":
                ...
                if len(
                    list(set([x[0] for x in outputs["additional_fastas_raw"]]))
                ...
                for extra_gene in outputs["additional_fastas_raw"]:
                    ...
                    if "*" in extra_gene[0]:
                        ...
                        outputs["additional_fastas"].append(
                            (extra_gene[0] + "*01", extra_gene[1])
                        )
            ...
            for ref_chain in ["TR1", "TR2"]:
                ...
                with warnings.catch_warnings(record=True) as chain_log:
                    ...
                    if (
                        values[ref_chain + "V"]
                        and values[ref_chain + "J"]
                        and values[ref_chain + "_CDR3"]
                    ):
                                ...
                                for extra_gene in outputs["additional_fastas"]:
                                    ...
                                    for gene_type in tcr_dat.keys():
                                        if gene not in tcr_dat[gene_type]:
                                            tcr_dat[gene_type][gene] = coll.defaultdict(
                                                list
                                            )
                                        ...
                                        if allele in tcr_dat[gene_type][gene]:
                                            raise warnings.warn(
                                                ...
                                                + extra_gene[0]
                                                + " already exists in TCR germline data. "
                                                "Please change and try."
                                            )
                                        else:
                                            tcr_dat[gene_type][gene][
                                                allele
                                            ] = extra_gene[1].upper()
                        ...
                        values[ref_chain + "V"]
                        or values[ref_chain + "J"]
                        or values[ref_chain + "_CDR3"]
                    ):
                        warnings.warn(
                            "V gene, J gene, and CDR3 sequence are all required to stitch a TCR chain."
                        )

                warning_msgs[ref_chain + "_out"] += "".join(
                    [
                        str(chain_log[x].message)
                        for x in range(len(chain_log))
            ...
            if values["chk_linker"]:
                with warnings.catch_warnings(record=True) as link_log:
                    ...
                    try:
                        if "TR1_out_str" in outputs and "TR2_out_str" in outputs:
                            # Determine order
                            if (
                                values["link_order_choice"] == "BA"
                                or values["link_order_choice"] == "DG"
                            ):
                                tr1, tr2 = "2", "1"
                            elif (
                                values["link_order_choice"] == "AB"
                                or values["link_order_choice"] == "GD"
                            ):
                                tr1, tr2 = "1", "2"
                            else:
                            ...
                            if outputs["linker"] == "Custom":
                                if values["custom_linker"]:
                                    ...
                                    window["linked_log"].update(
                                        "Cannot output linked sequence: custom linker chosen, "
                                        "but not provided"
                                    )

                            outputs["linker_seq"] = fxn.get_linker_seq(
                                outputs["linker"], linkers
                            )

                            outputs["linked"] = (
                                outputs["TR" + tr1 + "_stitched"]
                                + outputs["linker_seq"]
                                + outputs["TR" + tr2 + "_stitched"]
                            )

                            outputs["linked_header"] = "_".join(
                                [
                                    outputs["TR" + tr1 + "_out_str"],
                                    outputs["linker"],
                                    outputs["TR" + tr2 + "_out_str"],
                                ]
                            )

                            outputs["linked_fasta"] = fxn.fastafy(
                                outputs["linked_header"], outputs["linked"]
                            )
                            ...
                            raise warnings.warn(
                                "Two valid chains required for linking."
                            )
                ...
                warning_msgs["linked_out"] += "".join(
                    [
                        str(link_log[x].message)
                        for x in range(len(link_log))
            ...
            if "outputs" in dir():
                if len(outputs) > 0:
                    ...
                    if values["chk_linker"] and (
                        "TR1_out_str" in outputs and "TR2_out_str" in outputs
                    ):
