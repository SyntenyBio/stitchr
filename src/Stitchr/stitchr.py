...
def args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Partial amino acid sequence, if "
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Name for TCR sequence. Optional. "
    ...
    for r in fxn.regions:
        ...
        if in_gene in tcr_info[fxn.regions[r]]:
            ...
            raise ValueError(
                "Error: a "
                + fxn.regions[r]
                + " sequence region has not been found for gene "
                + gene
            ...
            if allele == "01":
                ...
                if not tcr_info[fxn.regions[r]][gene][allele]:
                    ...
                    warnings.warn(
                        ...
                        + "*"
                        + new_allele
                        + " is being used instead - please double "
        ...
        if gene and allele:
            ...
            func_err_base = (
                ...
                + "*"
                + allele
                + " has a IMGT-assigned functionality of '"
            ...
            if functionality[gene][allele] == "(F)" and r == "j":
                cdna_j_err = (
                    func_err_base
                    + "meaning it was only detected in cDNA, and thus IMGT doesn't record its"
            ...
            raise ValueError(
                "Cannot find TCR sequence data for "
                + r.upper()
                + " gene: "
                + in_gene
                + "*"
    ...
    if input_type == "nt" and seamless:
        ...
        if len(v_overlap) < 10:
            ...
            if len(v_overlap_2) > 10:
                ...
                v_overlap = specific_args["cdr3_nt"][
                    : len(v_overlap) + len(v_overlap_2) + 1
                ]
        ...
        stitched_nt, stitched_trans = fxn.tidy_c_term(
            stitched_nt, False, c_motifs, used_alleles["c"]
        )
    ...
    # Otherwise run regular amino-acid based germline determination
        ...
        if (
            len(
                [
                    x
                    for x in list(set([x for x in specific_args["cdr3"]]))
        ...
        c_term_nt_inframe, c_term_aa = fxn.tidy_c_term(
            c_term_nt_raw, specific_args["skip_c_checks"], c_motifs, used_alleles["c"]
        )
        ...
        c_term_nt_trimmed, cdr3_c_end = fxn.determine_j_interface(
            specific_args["cdr3"][cdr3_n_offset:],
            c_term_nt_inframe,
            c_term_aa,
            len(done["j"]),
            j_warning_threshold,
        ...
        if input_type == "nt":
            non_templated_nt = specific_args["cdr3_nt"][
                cdr3_n_offset * 3 : (cdr3_n_offset + cdr3_c_end) * 3
            ]
        else:
...
def main():
    ...
    codons = fxn.get_optimal_codons(
        input_args["codon_usage_path"], input_args["species"]
    )
    ...
    if input_args["preferred_alleles_path"]:
        preferred_alleles = fxn.get_preferred_alleles(
            input_args["preferred_alleles_path"], gene_types, imgt_dat, partial, chain
        )
    else:
        preferred_alleles = {}

    out_list, stitched, offset = stitch(
        input_args,
        imgt_dat,
        tcr_functionality,
        partial,
    ...
    if "_FA" in input_args["mode"]:
        ...
        if input_args["mode"] == "BOTH_FA" or input_args["mode"] == "AA_FA":
            ...
            print(
                fxn.fastafy("aa|" + out_str, fxn.translate_nt("N" * offset + stitched))
            )
    ...
    if input_args["aa"]:
        ...
        for i in range(0, 600, 60):
            ...
            if i > len(alignments[0][0]):
                break
            for y in [
                x[i : i + 60] for x in format_alignment(*alignments[0]).split("\n")[:3]
            ]:
                print(y)
