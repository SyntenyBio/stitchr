...
def args():
    ...
    parser.add_argument(
        "--cite", action="version", help="Print citation details.", version=fxn.citation
    )
...
convert_fields = {
    ...
    "TRB_leader": "l",
    "TCR_name": "name",
    "TRA_5_prime_seq": "5_prime_seq",
    "TRA_3_prime_seq": "3_prime_seq",
    "TRB_5_prime_seq": "5_prime_seq",
    "TRB_3_prime_seq": "3_prime_seq",
    "TRGV": "v",
    "TRGJ": "j",
    "TRG_CDR3": "cdr3",
    "TRDV": "v",
    ...
    "TRG_leader": "l",
    "TRD_leader": "l",
    "TRG_5_prime_seq": "5_prime_seq",
    "TRG_3_prime_seq": "3_prime_seq",
    "TRD_5_prime_seq": "5_prime_seq",
    "TRD_3_prime_seq": "3_prime_seq",
}

pre_stitch_list_fields = [
    "name",
    "v",
    "j",
    "c",
...
def main():
    ...
    if input_args["species"]:
        if input_args["species"].upper() in fxn.find_species_covered():
            ...
            raise IOError(
                "No data available for requested species: " + input_args["species"]
            )
    else:
    ...
    if input_args["receptor"]:
        ...
        if ("A" in input_receptor or "B" in input_receptor) and not (
            "G" in input_receptor or "D" in input_receptor
        ):
            receptor = "TRA/TRB"
        elif ("G" in input_receptor or "D" in input_receptor) and not (
            "A" in input_receptor or "B" in input_receptor
        ):
            receptor = "TRG/TRD"
        else:
            raise IOError(
                "Unable to determine receptor from '-r' command string: "
                + input_receptor
                + ". "
            )
        ...
        with fxn.opener(input_args["in_file"]) as in_file:
            for line in in_file:
                if "TRAV" in line and "TRGV" not in line:
                    ...
                    raise IOError(
                        "Unable to determine receptor from input file header, please check template. "
                    )
                break
    ...
    for c in [r1, r2]:
        tmp_tcr_dat, tmp_functionality, partial = fxn.get_imgt_data(
            c, st.gene_types, species
        )
        ...
        if "extra_genes" in input_args:
            if input_args["extra_genes"]:
                tcr_dat[c], tcr_functionality[c] = fxn.get_additional_genes(
                    tcr_dat[c], tcr_functionality[c]
                )
        ...
        if input_args["preferred_alleles_path"]:
            preferences[c] = fxn.get_preferred_alleles(
        ...
        for line in in_file:
            ...
            if line_count == 0:
                if bits != in_headers[receptor]:
                    raise ValueError(
                        "Headers in input file don't match the expectations - please check template."
                    )
                ...
                if len(bits) != len(
                    in_headers[receptor]
                ):  # Pad entries from tsvs where whitespace on right is missing
                ...
                if multiple_options:
                    ...
                    for multi_var in multiple_options:
                        ...
                        # If not a list, it should be a wild card, but only for specified fields
                            ...
                            entry_line_warnings.append(
                                "% wild cards only allowed in V/J/C/leader fields. "
                            )
                            break
                        ...
                        # Wild card just all alleles for a certain gene
                            ...
                            if gene in tcr_dat[var_name[:3]][region]:
                                var_options = [
                                    gene + "*" + x
                                    for x in tcr_dat[var_name[:3]][region][gene].keys()
                                ]
                            else:
                                entry_line_warnings.append(
                                    "Specified multiple entry option gene '"
                                    + gene
                                    + "' is not "
                        ...
                        # Or wild card all genes/allele combinations for a given region (AKA throw it all at the wall)
                            ...
                            entry_line_warnings.append(
                                "Unexpected multi-option field entry detected: "
                                + bits[multi_var]
                                + ". "
                            )
                            break
                ...
                if entry_line_warnings:
                    ...
                    out_data.append(
                        "\t".join([tcr_lines[0][x] for x in out_headers[receptor]])
                    )
                    continue
                ...
                for sorted_row_bits in tcr_lines:
                    ...
                    for c in [r1, r2]:
                        with warnings.catch_warnings(record=True) as chain_warnings:
                            ...
                            tcr_bits = {
                                "skip_c_checks": input_args["skip_c_checks"],
                                "species": species,
                            }
                            ...
                            if len(tcr_bits) > 1:
                                ...
                                if len(featured_bits) == 0:
                                    ...
                                    tcr_bits = fxn.autofill_input(
                                        populate_blanks(
                                            tcr_bits, pre_stitch_list_fields
                                        ),
                                        c,
                                    )
                                    tcr_bits = fxn.tweak_thimble_input(tcr_bits)
                                    try:
                                        out_list, stitched, offset = st.stitch(
                                            tcr_bits,
                                            tcr_dat[c],
                                            tcr_functionality[c],
                                            partial,
                                        ...
                                        sorted_row_bits[c + "_aa"] = fxn.translate_nt(
                                            "N" * offset + stitched
                                        )
                                            ...
                                            dict(
                                                list(
                                                    zip(
                                                        [
                                                            c + x
                                                            for x in post_stitch_list_fields
                                                        ],
                                                        out_list,
                                                    )
                                                )
                                        ...
                                        sorted_row_bits["Warnings/Errors"] += (
                                            "(" + c + ") " + str(message)
                                        )
                                        sorted_row_bits["Warnings/Errors"] += (
                                            "Cannot stitch a sequence for " + c + ". "
                                        )
                            ...
                            sorted_row_bits["Warnings/Errors"] += " ".join(
                                [
                                    "(" + c + ") " + str(chain_warnings[x].message)
                                    for x in range(len(chain_warnings))
                                    if "DeprecationWarning"
                                    not in str(chain_warnings[x].category)
                                ]
                            )
                    ...
                    with warnings.catch_warnings(record=True) as link_warnings:
                        ...
                        if sorted_row_bits["Linker"]:
                            if sorted_row_bits["Link_order"]:
                                sorted_row_bits["Link_order"] = sorted_row_bits[
                                    "Link_order"
                                ].upper()

                                # Only allow valid orders
                                if sorted_row_bits["Link_order"] not in [
                                    r2[2] + r1[2],
                                    r1[2] + r2[2],
                                ]:
                                    warnings.warn(
                                        "Error: given link order not valid (not "
                                        + r1[2]
                                        + r2[2]
                                        + " or "
                                        + r2[2]
                            ...
                            tr1, tr2 = (
                                sorted_row_bits["Link_order"][0],
                                sorted_row_bits["Link_order"][1],
                            )

                            if (
                                sorted_row_bits[r1 + "_nt"]
                                and sorted_row_bits[r2 + "_nt"]
                            ):
                                try:
                                    linker_seq = fxn.get_linker_seq(
                                        sorted_row_bits["Linker"], linker_dict
                                    )

                                    linked = (
                                        sorted_row_bits["TR" + tr1 + "_nt"]
                                        + linker_seq
                                        + sorted_row_bits["TR" + tr2 + "_nt"]
                                    )
                                    sorted_row_bits["Linked_nt"] = linked
                                    sorted_row_bits["Linked_aa"] = fxn.translate_nt(
                                        "N" * offset + linked
                                    )

                                    # Add warnings if sequences applied at maybe the wrong ends of things
                                    if (
                                        sorted_row_bits[r1 + "_5_prime_seq"]
                                        and sorted_row_bits["Link_order"]
                                        == r2[2] + r1[2]
                                    ) or (
                                        ...
                                        warnings.warn(
                                            "Warning: "
                                            + sorted_row_bits["Link_order"]
                                            + " order specified, "
                                            "but 3' chain has an additional 5' sequence provided. "
                                        )

                                    if (
                                        sorted_row_bits[r1 + "_3_prime_seq"]
                                        and sorted_row_bits["Link_order"]
                                        == r1[2] + r2[2]
                                    ) or (
                        ...
                        if sorted_row_bits["Warnings/Errors"] or link_warnings:
                            sorted_row_bits["Warnings/Errors"] += " ".join(
                                [
                                    "(Link) " + str(link_warnings[x].message)
                                    for x in range(len(link_warnings))
                                    if "DeprecationWarning"
                                    not in str(link_warnings[x].category)
                                ]
                            )
                        else:
                    ...
                    out_data.append(
                        "\t".join([sorted_row_bits[x] for x in out_headers[receptor]])
                    )
